...
package Paws::DynamoDB::Query;
  ...
  has KeyConditions => (is => 'ro', isa => InstanceOf['Paws::DynamoDB::KeyConditions']);
  has Limit => (is => 'ro', isa => Int);
  has ProjectionExpression => (is => 'ro', isa => Str);
  has QueryFilter => (is => 'ro', isa => InstanceOf['Paws::DynamoDB::FilterConditionMap']);
  ...
  sub new_with_coercions {
    ...
    if (exists $args->{KeyConditionExpression}) {
      $res{KeyConditionExpression} = (map {
            "$_"
      } ($args->{KeyConditionExpression}))[0];
    }
    if (exists $args->{KeyConditions}) {
      $res{KeyConditions} = (map {
            ref($_) eq 'Paws::DynamoDB::KeyConditions' ? $_ : do {
              require Paws::DynamoDB::KeyConditions;
              Paws::DynamoDB::KeyConditions->new_with_coercions($_);
              }
      } ($args->{KeyConditions}))[0];
    }
    ...
    if (exists $args->{ScanIndexForward}) {
      $res{ScanIndexForward} = (map {
            0 + !!$_
      } ($args->{ScanIndexForward}))[0];
    }
  ...
  sub new_from_xml {
    ...
    for ($xml->childNodes) {
      if (!defined(my $nodeName = $_->nodeName)) {
        ...
        my $key = "IndexName";
            $res->{$key} = "" . ( $_->nodeValue // '' );
      } elsif ($nodeName eq "KeyConditionExpression") {
        my $key = "KeyConditionExpression";
            $res->{$key} = "" . ( $_->nodeValue // '' );
      } elsif ($nodeName eq "KeyConditions") {
        my $key = "KeyConditions";
            do {
              require Paws::DynamoDB::KeyConditions;
              Paws::DynamoDB::KeyConditions->read_xml( $_, $res, $key );
            };
        ...
        my $key = "ReturnConsumedCapacity";
            $res->{$key} = "" . ( $_->nodeValue // '' );
      } elsif ($nodeName eq "ScanIndexForward") {
        my $key = "ScanIndexForward";
            $res->{$key} =
              do { my $d = $_->nodeValue // ''; $d eq "true" || $d eq "1" };
  ...
  sub to_hash_data {
    ...
    if (exists $self->{KeyConditionExpression}) {
      $res{KeyConditionExpression} = (map {
            "$_"
      } ($self->KeyConditionExpression))[0];
    }
    if (exists $self->{KeyConditions}) {
      $res{KeyConditions} = (map {
            $_->to_hash_data
      } ($self->KeyConditions))[0];
    }
    ...
    if (exists $self->{ScanIndexForward}) {
      $res{ScanIndexForward} = (map {
            0 + !!$_
      } ($self->ScanIndexForward))[0];
    }
  ...
  sub to_parameter_data {
    ...
    if (exists $self->{KeyConditionExpression}) {
      my $key = "${prefix}KeyConditionExpression";
      do {
            $res->{$key} = "$_";
      } for $self->KeyConditionExpression;
    }

    if (exists $self->{KeyConditions}) {
      my $key = "${prefix}KeyConditions";
      do {
            $_->to_parameter_data( $res, $key );
      } for $self->KeyConditions;
    }
    ...
    if (exists $self->{QueryFilter}) {
      my $key = "${prefix}QueryFilter";
      do {
            $_->to_parameter_data( $res, $key );
      } for $self->QueryFilter;
    }
    ...
    if (exists $self->{ScanIndexForward}) {
      my $key = "${prefix}ScanIndexForward";
      do {
            $res->{$key} = $_ ? "true" : "false";
      } for $self->ScanIndexForward;
    }
